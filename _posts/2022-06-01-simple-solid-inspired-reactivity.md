...
The [Solid in 100 seconds video][100secsolid] is quite neat, but I felt I needed some more, so I read [this article][introsolid][^1] about intro to Solid for React developers and watched the [advanced intro][solid10min].
...
This is thanks to Solid's reactivity. But wait, what is reactivity?
...
There's no dependency array in `createEffect` — Solid already knows to rerun this when `price` or `quantity` changes![^2]
...
It all starts with a simple state, similar to React's own `useState`. Let's call this `createSignal`, and implement a basic read and write:
...
To do that, let's maintain a global stack called `context` which lets us keep track of the effect that is currently running, and a function to get the current effect we're running in:
...
I hope that this simple exercise of implementing reactivity in JS excited you as much as it blew my mind—look at how, by just using the old plain stack, we managed to inject reactivity into our code. 🤯
